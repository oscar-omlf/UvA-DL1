...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        pos_emb = torch.cat([freqs, freqs], dim=-1)[None, None, :, :]  # Shape: (1, 1, T, dim)
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer_norm_1 = RMSNorm(n_embd)
        self.self_attention = CausalSelfAttention(config)

        self.layer_norm_2 = RMSNorm(n_embd)
    ...
    def forward(self, x):
        ...
        x_att = x + self.self_attention(norm_1)

        norm_2 = self.layer_norm_2(x_att)
        out = x_att + self.mlpf(norm_2)
        return out
        ...
        for _ in range(max_new_tokens):
            ...
            if not do_sample:
                ...
                if top_k is not None:
                    ...
                    idx_next = topk_idx.gather(-1, next_local)      
                ...
                # optionally apply top-p sampling
                    ...
                    idx_next = sorted_idx.gather(-1, next_local)  
